...
########################################################################
PROJECTD=$(shell pwd)
PROJECT=$(notdir $(PROJECTD))

TOP=$(shell echo $(CURDIR) | sed -e 's+/Tests/.*+/Tests+')
include $(TOP)/src/Makefile.tests

backup:
	tar cf - --directory=$(HOME) projects/$(PROJECT) | bzip2 -9 > $(HOME)/$(PROJECT).tbz2
